...
- **Turkey, Nigeria, Pakistan** thua lỗ nặng — cần review chiến lược discount tại các thị trường này
- Tổng 27 quốc gia đang thua lỗ — chiếm 18% tổng số thị trường

---

#### Chart 16 — Market Performance Dashboard (4 charts)
...
- Một số thành phố tại Trung Đông và châu Phi ghi nhận profit âm mặc dù có doanh thu ổn
...
- **Tháng 11** luôn là tháng cao nhất mọi năm — peak Black Friday/Holiday season
...
**Vấn đề:** File `Global_Superstore2.xlsx` chỉ có 1 sheet tên `Sheet1`, không phải `Orders/Returns/People` như tài liệu tham khảo mô tả.

**Giải pháp:** Dùng `pd.ExcelFile().sheet_names` để auto-detect tên sheet thay vì hardcode. Đọc sheet đầu tiên bất kể tên gì.
...
**Vấn đề:** Cột Order Date định dạng `dd/mm/yyyy` (UK format) — Power BI mặc định đọc là `mm/dd/yyyy` (US format) → các ngày > 12 bị lỗi "Error".

**Giải pháp:** Dùng "Using Locale → English (United Kingdom)" khi đổi kiểu dữ liệu trong Power Query.
...
**Vấn đề:** `__file__` (2 dấu gạch dưới mỗi bên) bị render thành `_file_` khi copy từ chat → `NameError`.

**Giải pháp:** Dùng đường dẫn hardcode tuyệt đối thay vì `os.path.dirname(__file__)`.
...
**Vấn đề:** DAX measure `Avg Delivery Days = AVERAGE(DATEDIFF(...))` báo lỗi — AVERAGE chỉ nhận column reference.

**Giải pháp:** Dùng `AVERAGEX()` để tính theo từng dòng.
...
**Vấn đề:** Power BI Filled Map không cho phép kéo Measure (như Total Sales) vào ô Legend — chỉ nhận column text/category.

**Giải pháp:** Dùng `Orders[Market]` (cột text) vào Legend để tô màu theo Market. Thêm Measures vào Tooltips để xem khi hover.

---

### Issue 6 — Git push lỗi "Repository not found"

**Vấn đề:** Tên repo trên GitHub (`GlobalSuperstore_Analysis`) khác với URL trong lệnh git (`global-superstore-analysis`) — case-sensitive và format khác nhau.
...
**Bài học:** Luôn copy chính xác URL từ trang GitHub repo thay vì gõ tay.
...
Bằng chứng từ 51,290 đơn hàng thực tế cho thấy mối quan hệ trực tiếp và đo lường được giữa mức discount và profit margin. Việc cap discount ở 20% có thể cứu hầu hết các sub-category đang thua lỗ mà không cần thay đổi chiến lược sản phẩm.

**2. 22% khách hàng Champions tạo ra 61% doanh thu — đây là tài sản cần bảo vệ ưu tiên.**
RFM analysis xác nhận rõ ràng Pareto 80/20. Chi phí giữ 1 Champions hiệu quả hơn 5–8x so với acquisition khách hàng mới. Chiến lược VIP program cho Champions là investment có ROI cao nhất.

**3. Q4 peak và Jan/Feb trough là pattern ổn định — đủ độ tin cậy để lập kế hoạch.**
4 năm liên tiếp xác nhận seasonality pattern. Đây không phải may mắn mà là cấu trúc thị trường. Doanh nghiệp có thể dùng pattern này để tối ưu inventory và staffing với độ chắc chắn cao.
...
| **Python EDA** | 5 notebooks, 20 charts, 3 CSV exports có thể tái sử dụng |
...
- **Streamlit Dashboard:** Deploy web app tương tác tại [streamlit.io/cloud](https://streamlit.io/cloud) — không cần Power BI license
- **Machine Learning:** Dự báo discount tối ưu cho từng sub-category bằng Linear Regression
- **Customer Churn Prediction:** Dùng Random Forest dự báo khách hàng nào sắp chuyển từ Loyal → At Risk
- **Real-time Pipeline:** Kết nối với Google Sheets/API để dashboard tự động refresh
...
Dataset thuộc sở hữu của [Tableau](https://www.tableau.com/) và được công bố công khai trên Kaggle. Mọi phân tích và code trong repo này được phát hành theo **MIT License**.
